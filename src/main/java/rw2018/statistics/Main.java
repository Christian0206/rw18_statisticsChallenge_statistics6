...
public class Main {
	...
	private static void collectStatistics(File statisticsDir, File[] chunks) {
		...
		try (StatisticsDB statisticsDB = new StatisticsDBImpl();) {
			...
			for (int chunkI = 0; chunkI < chunks.length; chunkI++) {
				...
				try (EncodedFileInputStream input = new EncodedFileInputStream(EncodingFileFormat.EEE, chunk);) {
					for (Statement stmt : input) {
						statisticsDB.incrementFrequency(stmt.getSubjectAsLong(), chunkI, TriplePosition.SUBJECT);
						statisticsDB.incrementFrequency(stmt.getPropertyAsLong(), chunkI, TriplePosition.PROPERTY);
	...
	public static void main(String[] args) throws ParseException {
		...
		Option working = Option.builder("w").longOpt("workingDir").hasArg().argName("workingDirectory")
				.desc("the working directory in which the statistics database will be persisted").required(true)
				.build();
